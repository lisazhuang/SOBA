 ...
 */
public class SeminarAlgorithmLaptop extends AbstractAlgorithm2 {
	/* Weka. */
	 ...
	 * @param analysisSpan, the span type used to split the data (sentence)
	 ...
	 */
	public SeminarAlgorithmLaptop(String analysisSpan, String evalAnalysisSpan, boolean failureAnalysis, boolean printOutput, int num, String ont) {
	...
	public void preprocess() {		
		...
		for (Span sentence : getCombinedData()) {
			...
			for (Span span : sentence.getDataset().getSpans(sentence, "opinion")) {
				...
				if (span.getAnnotations().containsKey("polarity")) {
					goldValues.put(span, span.getAnnotations().get("polarity", String.class));
				}
				...
				/*Get the category*/
				String category = span.getAnnotations().get("category", String.class);
				...
				for (Word word : scope) {
					...
					if (hasProperty("applyWords")) {
						...
						if (hasProperty("applyStanfordSentiment")){
							double sentiment = getSentimentScore(word.getAnnotations().get("phraseSentiment", ArrayList.class));
					...
					if (hasProperty("applyOntology") && word.getAnnotations().containsKey("URI") && !word.getAnnotations().get("URI").equals("null")) {
						...
						if (hasProperty("applyNegation")) {
							...
							while (word2.hasPreviousWord() && !punctuation){
								...
								String prevLemma = prev.getAnnotations().get("lemma", String.class);
								...
								else if(prevLemma.equals("none")|| prevLemma.equals("too") || prevLemma.equals("nobody")|| prevLemma.equals("never") ||  prevLemma.equals("nothing") || prevLemma.equals("no") || prevLemma.indexOf("n't")!=-1) {
									negatedtimes++;
								}
								word2 = prev;
							}
									...
									for (Word relWord : relatedWords){
										...
										if (relWord.getAnnotations().containsKey("URI")){
											...
											if (classes.contains(ont.URI_AspectMention)){
												...
												String newClassURI = ont.addClass(word1 + " " + word2, true, word1 + " " + word2, URI, relWord.getAnnotations().get("URI"));
...
//						System.out.println(word.getLemma() + "::"+ word.getTextualUnit().getAnnotations().get("id")+" :: "+foundURIs);
				...
				if (hasProperty("applyStanfordReviewSentiment")){
					...
					double stanfordSentiment = 0.0;
						...
						double sentenceScore = getSentimentScore(sentence.getAnnotations().get("phraseSentiment", ArrayList.class));
	...
	public void train() {
		...
		try {
			...
			for (int i = 0; i < trainingSpans.size(); i++) {
				...
				p.getAnnotations().put("polarity", classLabels.get((int) inSamplePredictions[i]));
	...
	public void predict() {
		...
		try {
			...
			for (int i = 0; i < testSpans.size(); i++) {
				...
				Prediction p = new Prediction(s);
				p.getAnnotations().put("polarity", classLabels.get((int) outSamplePredictions[i]));
				this.predictions.put(s, p.getSingletonSet());
			}
