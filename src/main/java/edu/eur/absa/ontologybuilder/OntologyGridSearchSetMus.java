 ...
 *
 */
public class OntologyGridSearchSetMus {
	 ...
	 */
	public OntologyGridSearchSetMus(SeminarOntology baseOnt, HashMap<String, HashSet<String>> aspectCat, String dom, double thres, double frac, boolean r) {
	 ...
	 * @param frac, the top fraction of terms to suggest
	 */
	public OntologyGridSearchSetMus(SeminarOntology baseOnt, HashMap<String, HashSet<String>> aspectCat, String dom, double thres, double invThres, double frac, boolean r) {
	 ...
	 */
	public double findTerms(boolean nn, boolean adj, boolean vrb, double alpha, double beta, double fraction_verbs, double fraction_nouns, double fraction_adj, HashMap<String, Double> acceptedWords) throws ClassNotFoundException, JSONException, IllegalSpanException, IOException {

		fraction[0] = fraction_verbs;
		fraction[1] = fraction_nouns;
		fraction[2] = fraction_adj;
		...
		if (vrb) {
			...
			Arrays.sort(scores);
			double ind = fraction[0] * verbs.size(); //TODO: what is fraction?
			int index = (int) ind;			
		...
		if (nn) {
			...
			/* Find the threshold value to get only the top n% of the terms. */
			Arrays.sort(scores);
			double ind = fraction[1] * nouns.size();
			int index = (int) ind;
		...
		if (adj) {
			...
			/* Find the threshold value to get only the top n% of the terms. */
			Arrays.sort(scores);
			double ind = fraction[2] * adjectives.size();
			int index = (int) ind;			
