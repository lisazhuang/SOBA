 ...
 *
 */
public class OntologyGridSearchSetT {
	 ...
	 */
	public OntologyGridSearchSetT(SeminarOntology baseOnt, HashMap<String, HashSet<String>> aspectCat, String dom, double thres, double[] fraction, boolean r) {
		this(baseOnt, aspectCat, dom, thres, thres, fraction, r );
	}
	 ...
	 * @param frac, the top fraction of terms to suggest
	 */
	public OntologyGridSearchSetT(SeminarOntology baseOnt, HashMap<String, HashSet<String>> aspectCat, String dom, double thres, double invThres,double[] frac, boolean r) {
	 ...
	 */
	public HashMap<String, Double> findTerms(boolean nn, boolean adj, boolean vrb, double alpha, double beta, HashMap<String, Double> acceptedWords) throws ClassNotFoundException, JSONException, IllegalSpanException, IOException {
		...
		int max_length = 0;
		HashMap<String, Double> return_words = new HashMap<String, Double>();
		...
		if (vrb) {
			...
			Arrays.sort(scores);
			double ind = fraction[2] * verbs.size(); //TODO: what is fraction?
			int index = (int) ind;			
			...
			for (String w : verbs.keySet()) {
				...
				if (score > scoreThreshold) {
					if (acceptedWords.containsKey(w)) { //aspectverb TODO: what is accpetedWords?
						return_words.put(w, verbs.get(w));
						numVerbsAccept++;
						numAccept++;
						numAcceptOverall++;
					} else {
		...
		if (nn) {
			...
			/* Find the threshold value to get only the top n% of the terms. */
			Arrays.sort(scores);
			double ind = fraction[0] * nouns.size();
			int index = (int) ind;
			...
			for (String w : nouns.keySet()) {
				...
				if (score > scoreThreshold) {
					if (acceptedWords.containsKey(w)) {
						...
						a.add(w);		//added
						return_words.put(w, nouns.get(w));
						numNounsAccept++;
						numAccept++;
						numAcceptOverall++;
						nouns.put(w, 0.0);
		...
		if (adj) {
			...
			/* Find the threshold value to get only the top n% of the terms. */
			Arrays.sort(scores);
			double ind = fraction[1] * adjectives.size();
			int index = (int) ind;			
			...
			for (String w : adjectives.keySet()) {
				...
				if (score > scoreThreshold) {
					if (acceptedWords.containsKey(w)) {
						return_words.put(w, adjectives.get(w));
						numAdjectivesAccept++;
						numAccept++;
						numAcceptOverall++;
					} else {
		...
		numAccept_words = numAccept;
		numReject_words = numReject;
	 ...
	 */
	public double subsumptionAspect(HashMap<String, Double> words, HashMap<String, HashSet<String>> relations, double t) {		//yes to noun, pos = "noun" or "verb" or "adjective"
		HashSet<String> set;
		HashMap<String, String> acceptedRelations = new HashMap<String, String>();
		numAccept = numAccept_words;
		numReject = numReject_words;
		numRejectOverall = numReject;
		numAcceptOverall = numAccept;
		...
		for (String word : words.keySet()) {
			...
			for (String parentURI : set) {
				...
				if (wordProb >= t && wordProbInv < invThreshold ) {
			...
			for (double parentScore : scoreParents.keySet()) {
				if (!accept) { //not accepted as either negative of positive generic.
					...
					if (relations.get(word).contains(finalParentURI))
					{
						numAcceptOverall++;
		...
		//System.out.println("Threshold = " + t + " // numAccept = " + numAcceptOverall + " // numReject: " + numRejectOverall + " // Ratio: " + (numAcceptOverall/(numAcceptOverall + numRejectOverall)));
 		return numAcceptOverall/(numAcceptOverall + numRejectOverall);
		}
	...
	public int[] getStats() {
		...
		stats[0] = numAccept;
		stats[1] = numReject;
		stats[2] = numAcceptOverall;
		stats[3] = numRejectOverall;
		return stats;
	}
