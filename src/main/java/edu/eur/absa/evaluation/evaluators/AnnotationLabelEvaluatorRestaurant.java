...
public class AnnotationLabelEvaluatorRestaurant implements Evaluator {
	...
	public AnnotationLabelEvaluatorRestaurant(String spanType, String annotationType, boolean failAn){
		this(spanType, annotationType, failAn, null);
	}
	
	public AnnotationLabelEvaluatorRestaurant(String spanType, String annotationType, boolean failAn, SeminarWriter writer){
...
//					Main.debug("No prediction found for: "+parentSpan);
		...
		if (writeOutput) {
			try {
				outputWriter.write(null, new File(Framework.DATA_PATH+"ThesisRestaurant.predictionsALLTest.json"));
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
